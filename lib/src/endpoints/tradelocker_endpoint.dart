...
class TradeLockerEndpoint extends Endpoint {
  ...
  Future<LinkedAccountDto> getAccountDto(Session session, LinkedAccount linkedAccount) async {
    // var authenticated = await session.authenticated;
    ...
    if (statusCurrency.isEmpty) {
      throw GeneralTradelyException('No status/currency found for this account');
    }

    //List<int> statuses = statusCurrency.map((x) => x.status).toList();
    List<String> currency = statusCurrency.map((x) => x.currency).toList();
    List<double> balance = statusCurrency.map((x) => double.parse(x.accountBalance)).toList();
